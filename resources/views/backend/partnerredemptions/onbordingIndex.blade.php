...
@section ('title', trans('Partener on-bording Management'))

@section('page-header')
    <h1>{{ trans('Partener on-bording Management') }}</h1>
@endsection

@section('content')
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('Partener on-bording Management') }}</h3>

            <div class="box-tools pull-right">
                
                <button type='button' class="btn btn-primary btn-md" id="generatePdf">Generate</button>
                <button type='button' class="btn btn-primary btn-md" id="processed">Processed</button>
            </div>
    ...
    <script>
        ...
        $(function() {
            ...
            var dataTable = $('#partnerredemptions-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.partnerredemptions.onbordingget") }}',
                    type: 'post'
                },
                columns: [
                    {data: null, name: 'checkbox', searchable: false, sortable: false,
                        render: function ( data, type, row, meta ) {
                            return "<input type='checkbox' value="+row.id+" data-set="+row.id+" >";
                        }
                    },
            ...
            $(document).on("click", "#processed", function () {
                ...
                $.ajax({
                url: "{{ url('admin/partnerredemption/processed') }}",
                type: "POST",
                data:{
                    "idArr": val,
                },
