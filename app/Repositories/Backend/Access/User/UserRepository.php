...
class UserRepository extends BaseRepository
{
    ...
    public function getForDataTable($status = 1, $trashed = false)
    {
        ...
        $dataTableQuery = $this->query()
            ...
            ->select([
                ...
                config('access.users_table').'.designation',
                config('access.users_table').'.contact_no',
                config('access.users_table').'.email',
                config('access.users_table').'.status_on',
    ...
    public function deleteAll($ids)
    {
        ...
        $result = DB::table('admin')->whereIn('id', $ids)->delete();
    ...
    // {
    //     //Figure out if email is not the same
    //     if ($user && $user->user_name === $input['user_name']) {
    //         return;
    //     }

    //     //Check to see if email exists
    //     if ($this->query()->where('user_name', '=', $input['user_name'])->withTrashed()->exists()) {
